...
//				dom.find(".ey-attrMeasureFooter").show();
			...
			dom.find(".ey-attributeAdd").bind("click",function(){
				if(!currentObj._value.colId){
					dom.find(".ey-attSortPullDown input").each(function(){
					...
					//数组状态设置
					 dom.find(".ey-attSortPullDown input").each(function(){
						 	$(this).prop("checked",false);
								 if($(this).prop("id") == currentObj._value.colId){
				...
				var hidePullDown = ey.eventLibrary.eventBase("hidePullDown");	
					currentObj.triggerEvent(hidePullDown); 
					 if(dom.find(".ey-attrMeasureFooter").is(":hidden")){
						 dom.find(".ey-attSortPullDown").css("margin-top","0px");
					 }
					 else{
						 dom.find(".ey-attSortPullDown").css("margin-top","-40px");
					 }
		...
		setCurrentValue : function(value){
			...
			if(value){
				this.refurbishData("selsect",value);
				this._content.find("input").each(function(){
					$(this).prop("checked",false);
					if($(this).val() == value.orderType){
						$(this).prop("checked",true);
					}
				});
				var dom = this._domTemplate;
				...
				this._domTemplate.find(".ey-attrMeasureFooter").show();
			}
		},
		...
		_bindEvent : function(dom){
			...
			dom.find(".ey-attributeSetting").bind("click",function(){
				var textValue =  $(this).parent().parent().find(".ey-controllerInputEditor").eq(0).text();
				var eyTooltip = ey.widget.eyTooltip({
						"title":"请选择排序",
						"content":currentObj._content,
						"className":'ey-tooltipSmall',
						"commit":function(){
							 var value = {};
							 value["colId"] = currentObj._domTemplate.find(".ey-controllerInputEditor").attr("value");
							 value["orderType"] =  currentObj._content.find("input:checked").val();
							 value["colAlias"]  = textValue;
							 value["value"] = true;
							 currentObj._value = value;
