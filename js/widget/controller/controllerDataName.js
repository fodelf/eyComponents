...
(function($,ey){
    ...
    var ControllerDataName = function(attribute){
		...
		this._tableContent=$("<div class='ey-datatypeContent'>"
            				...
            				+"<ul id='num_list' class='clearfix'></ul></div>"
            				+"<a id='ey-nextPage' href='javascript:;'>下一页</a></div>");
		this.layer = $("<div class='ey-dataQueryLayer' style ='position: absolute;margin-top: -40px;height:40px;width:240px;z-index:10;cursor: pointer;'></div>");
    };
    //继承组件基类
    ...
    ey.expandPrototype(ControllerDataName,{
		...
		_requestFun : function(currentObj){
			...
			$.ajax({
				...
				success:function(data){
					console.timeEnd("tableList");
					//重新绘制表格内容
					currentObj._redrawTbale(data);
					currentObj._drawPaging(data.resultEntity.totalRecord);
				}
			});
		},
		//设置对象属性值
		...
		_request :function(){
			...
			this.pageNum = 1;
			//初始化
			currentObj._tableContent.find("#ey-dataTypeSelect").text("全部");
			
			//解除之前的绑定事件
			this._domTemplate.find('input').unbind('click');
			//新事件绑定
			this._domTemplate.find('input').bind('click',function(){
				if(currentObj._domTemplate.find(".ey-dataQueryLayer").length ==0){
					currentObj._domTemplate.append(currentObj.layer);
				}
					...
					success:function(data){
						...
						var eyTooltip = ey.widget.eyTooltip({
							...
							"commit":function(){
								...
								currentObj.pageNum = 1;
								eyTooltip.close();
								currentObj._tableContent.find(".ey-dataTypeSelect").eq(0).unbind("click");
								currentObj._tableContent.find(".ey-dataTypeSelect").eq(0).bind("click",function(){
									currentObj._tableContent.find('.ey-dialogOption').hide();
									$(this).children('.ey-dialogOption').toggle();
									$(this).find('span').show();
								});
								currentObj._tableContent.find('.ey-dialogOption span').unbind("click");
								currentObj._tableContent.find('.ey-dialogOption span').click(function(ev){
									ev.stopPropagation();
									$(this).parent().siblings().text($(this).text());
									$(this).hide();
								...
								currentObj._tableContent.find(".ey-search").eq(0).unbind("click");
								//点击查询
								currentObj._tableContent.find(".ey-search").eq(0).click(function(){
									currentObj.pageSize = 5;
									currentObj.pageNum = 1;
		...
		_drawPaging :function(cont){
			...
			this._tableContent.find("#num_list").text("");
 			...
 			this._tableContent.find(".ey-pageNum").css("width",this.len * 24 + 1 + "px");
			...
			this.currentPage = currentObj._tableContent.find("#num_list").find("li").eq((this.pageNum-1));
			//初始化
		...
		_pagination : function(){
			...
			if(this.len>7){
				if( this.pageNum>3 && this.pageNum<this.len-4 ){
					currentObj._tableContent.find("#num_list").css("marginLeft",-((currentObj.pageNum-3)*24)+"px");
				}else if(this.pageNum<=3){
					currentObj._tableContent.find("#num_list").css("marginLeft","0px");
				}else if( this.pageNum>=this.len-4 ){
					currentObj._tableContent.find("#num_list").css("marginLeft",-((currentObj.len-8)*24)+"px");
				}
		...
		_current :function(){
		    ...
		    this._tableContent.find("#num_list").find("li").eq(that.pageNum - 1).attr("class","active");
		    this.currentPage = this._tableContent.find("#num_list").find("li").eq(this.pageNum);
		},
		...
		refurbish : function(refurbishValue,styleValue){
			...
			if(refurbishValue == "source_dataTable"  || refurbishValue == "source_workTable" || refurbishValue == "source_SQL"){
