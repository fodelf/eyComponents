...
(function($,ey,echarts){
	"use strict";
    var EyBarChartsColors = function(args){
    	//继承线图基类
    	...
    	this._domTemplate = $("<div class='ey-lineCharts ey-widget eyCharts eyCharts-bg-blue'>"
    						...
    						+"</div>");
    	this.widgetName = "eyBarChartsColors";
    	this.lineColor = [{"full":"#209c91","empty":"rgba(32,156,146,.4)"}];
    };
    //继承组件基类
    ey.extendFun(EyBarChartsColors,"eyCharts","widget");
    //扩展当前组件
    ey.expandPrototype(EyBarChartsColors,{
    	//设置图例类型
...
//  		this.option.series[0].itemStyle.normal.color = lineColor[0].full;
    		switch(this.lineType){
    			case 'full':
    				...
    				break;
    			case 'empty':
    				$.extend(true, option.series[0].itemStyle.normal,
						{'color':function(params) {
...
//  		alert(this.lineColor);
    ...
    ey.widget.eyBarChartsColors = function (args){
    	return new EyBarChartsColors (args);
    };
