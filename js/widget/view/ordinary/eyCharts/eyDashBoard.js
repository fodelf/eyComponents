...
(function($,ey,echarts){
    ...
    var EyDashBoard = function(args){
    	...
    	this._domTemplate = $("<div class='ey-lineCharts ey-widget eyCharts eyCharts-bg-blue'>"
    						...
    						+"</div>");
    	this.widgetName = "eyDashBoard";
    	this.lineColor = [{"full":"#209c91","empty":"rgba(32,156,145,.3)"}];
    };
    //继承组件基类
    ey.extendFun(EyDashBoard,"eyCharts","widget");
    //扩展当前组件
    ey.expandPrototype(EyDashBoard,{
    	//设置图例类型
...
//  				value.itemStyle.normal.borderColor = that.lineColor[i].full;
    	...
    	setLineColor:function(lineColor){
    		...
    		$.each(this.option.series[0].axisLine.lineStyle.color,function(index,value){
    			value[1] = lineColor[index].full
    		});
    	},
    ...
    ey.widget.eyDashBoard = function (args){
    	return new EyDashBoard (args);
    };
