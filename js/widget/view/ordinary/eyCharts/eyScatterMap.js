...
(function($,ey,echarts){
	"use strict";
    var EyScatterMap = function(args){
    	//继承线图基类
    ...
    //继承组件基类
    ey.extendFun(EyScatterMap,"eyCharts","widget");
    //扩展当前组件
    ey.expandPrototype(EyScatterMap,{
    	...
    	setSymbolColor:function(symbolColor){
    		this.symbolColor = symbolColor;
    		this.option.series[1].itemStyle.normal.color = symbolColor[0].full;
    	},
    	...
    	setOption : function(option){
			...
			this.option.series[0].data = this._convertData(data);
			this.option.series[0].symbolSize = function (val) {
                return val[2] / 20;
           	};
           	this.option.series[1].data = this._convertData(data.sort(function (a, b) {
					                return b.value - a.value;
					            }).slice(0, 6));
	        this.option.series[1].symbolSize = function (val) {
	            return val[2] / 20;
	        };			
		},
		...
		createSaticData:function(){
			...
			for(var i =0,len= dataArry.length;i<len;i++){
				...
				for(var k = 0 ,childLen = childArry.length;k <childLen;k++ ){
					if(k == 0){
						dataChild["name"] = childArry[k];				
					}
					else if(k == 1){
						dataChild["value"] = childArry[k];					
					}
				}
				data.push(dataChild);
			}
    ...
    ey.widget.eyScatterMap = function (args){
    	return new EyScatterMap (args);
    };
