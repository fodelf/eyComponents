...
(function($,ey,echarts){
	"use strict";
    var EyPerColumnCharts = function(args){
    	//继承线图基类
    	...
    	this._domTemplate = $("<div class='ey-lineCharts ey-widget eyCharts eyCharts-bg-blue'>"
    						...
    						+"</div>");
    	this.widgetName = "eyPerColumnCharts";
    	this.lineColor = [
    ...
    //继承组件基类
    ey.extendFun(EyPerColumnCharts,"eyCharts","widget");
    //扩展当前组件
    ey.expandPrototype(EyPerColumnCharts,{
    	setOption:function(option){
    		var options = JSON.stringify(option);			
			this.option = JSON.parse(options);
			this.option.yAxis[0].axisLabel.formatter = function(value,index) {
                return value + "%";
            }
    	},
    	//设置图例类型
    ...
    ey.widget.eyPerColumnCharts = function (args){
    	return new EyPerColumnCharts (args);
    };
