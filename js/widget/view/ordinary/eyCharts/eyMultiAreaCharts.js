...
(function($,ey,echarts){
	"use strict";
    var EyMultiAreaCharts = function(args){
    	//继承线图基类
    	...
    	this._domTemplate = $("<div class='ey-lineCharts ey-widget eyCharts eyCharts-bg-blue'>"
    						...
    						+"</div>");
    	this.widgetName = "eyMultiAreaCharts";
    	this._lineCount = 10;
    };
    //继承组件基类
    ey.extendFun(EyMultiAreaCharts,"eyCharts","widget");
    //扩展当前组件
    ey.expandPrototype(EyMultiAreaCharts,{
    	//每个图形必须重写此方法
    	getOption : function(){
			//清除无用数据只保留样式 非常重要-------------
...
//  	设置坐标轴文字大小
    ...
    ey.widget.eyMultiAreaCharts = function (args){
    	return new EyMultiAreaCharts (args);
    };
