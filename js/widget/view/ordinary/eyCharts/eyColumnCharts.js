...
(function($,ey,echarts){
	"use strict";
    var EyColumnCharts = function(args){
    	//继承线图基类
    	...
    	this._domTemplate = $("<div class='ey-lineCharts ey-widget eyCharts eyCharts-bg-blue'>"
    						...
    						+"</div>");
    	this.widgetName = "eyColumnCharts";
    	this.lineColor = [{"full":"#209c91","empty":"rgba(32,156,146,.4)"}];
    };
    //继承组件基类
    ey.extendFun(EyColumnCharts,"eyCharts","widget");
    //扩展当前组件
    ey.expandPrototype(EyColumnCharts,{
    	//每个图形必须重写此方法
    	getOption : function(){
			//清除无用数据只保留样式 非常重要-------------
...
//  		alert(this.lineColor);
    ...
    ey.widget.eyColumnCharts = function (args){
    	return new EyColumnCharts (args);
    };
