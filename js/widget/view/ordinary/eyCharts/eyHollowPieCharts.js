...
(function($,ey,echarts){
	"use strict";
    var EyHollowPieCharts = function(args){
    	//继承线图基类
    	...
    	this._domTemplate = $("<div class='ey-lineCharts ey-widget eyCharts eyCharts-bg-blue'>"
    						...
    						+"</div>");
    	this.widgetName = "eyHollowPieCharts";
    	this.lineColor = [{"full":"#209c91","empty":"rgba(32,156,145,.3)"}];
    };
    //继承组件基类
    ey.extendFun(EyHollowPieCharts,"eyCharts","widget");
    //扩展当前组件
    ey.expandPrototype(EyHollowPieCharts,{
    	//设置图例类型
    	...
    	setOutpieRadius : function(outpieRadius){
    		this.outpieRadius = outpieRadius;
    		this.option.series[0].radius[1] = outpieRadius;
    	},
    	getOutpieRadius : function(){
    		return this.outpieRadius;
    	},
    	setInpieRadius : function(inpieRadius){
    		this.inpieRadius = inpieRadius;
    		this.option.series[0].radius[0] = inpieRadius;
    	},
    	getInpieRadius : function(){
    		return this.inpieRadius;
    	},
...
//			$.extend(true, option, {'color':this._lineColor});
    ...
    ey.widget.eyHollowPieCharts = function (args){
    	return new EyHollowPieCharts (args);
    };
