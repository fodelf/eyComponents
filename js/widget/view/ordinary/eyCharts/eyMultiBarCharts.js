...
(function($,ey,echarts){
	"use strict";
    var EyMultiBarCharts = function(args){
    	//继承线图基类
    	...
    	this._domTemplate = $("<div class='ey-lineCharts ey-widget eyCharts eyCharts-bg-blue'>"
    						...
    						+"</div>");
    	this.widgetName = "eyMultiBarCharts";
    	this.lineColor = [
    ...
    //继承组件基类
    ey.extendFun(EyMultiBarCharts,"eyCharts","widget");
    //扩展当前组件
    ey.expandPrototype(EyMultiBarCharts,{
    	//每个图形必须重写此方法
    	getOption : function(){
			//清除无用数据只保留样式 非常重要-------------
...
//  		alert(this.lineColor);
    ...
    ey.widget.eyMultiBarCharts = function (args){
    	return new EyMultiBarCharts (args);
    };
